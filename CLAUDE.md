...
### 破壊的アクションの表示ルール
- 削除・上書き・移動など元に戻しにくい操作をユーザーに依頼・提案するときは、対象を名指しで一覧化し、アクション別に記号を付けて目立たせる:
  - 🗑️ **削除する**
  - ✅ **残す（触らない）**
  - ⚠️ **要判断（ユーザーの判断待ち）**
- 「削除する一覧」と「残す一覧」は必ず別の表・別のブロックに分け、1つの表に混在させない。
- 冒頭に件数を明記する（例:「🗑️ 削除9件 / ✅ 残す6件 / ⚠️ 要判断1件」）。
- 復元できない操作（バックアップ・複製が存在しないもの）には、その旨を太字で警告する。
